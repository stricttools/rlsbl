...
class TestTheRenameNeedsAChangelogEntry:
    """The rename commit carries no Autogenerated trailer, so changelog coverage
    decides whether it needs an entry, and the closing message prints the
    command that adds one exactly when coverage asks for it. With {name} in the
    tag format the commit changes the publish workflow, which coverage asks an
    entry for; a name-only rename touches only files rlsbl owns, which it does
    not.
    """

    def test_a_name_only_rename_prints_no_changelog_line_and_needs_no_entry(
            self, tmp_path, monkeypatch, _gh_ok):
        import rlsbl
        ...
        root.mkdir()
        monkeypatch.chdir(root)
        _build_monorepo(root, tag_format="v{version}", create_tag=False)
        _release(root, "beta", "0.1.0", tag_format="v{version}")
        ...
        assert "changelog" not in result.stdout.lower(), result.stdout

        after = coverage()
        assert after.exit_code == 0, after.stdout + after.stderr
