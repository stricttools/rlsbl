...
| `--version` |  | str | optional |  | The version whose archive is marked unrecoverable and whose release commit --commit supplies (e.g. 0.4.2, with or without a leading v). Only together with --commit; the whole-repository pass does not run. |
| `--commit` |  | str | optional |  | The commit --version shipped from, as any spelling git resolves (a full or abbreviated hash). It must be an ancestor of HEAD, and every target in the release's scope must declare --version at it. Only together with --version. |
| `--releasable` |  | str | optional |  | Which releasable's archive --commit is recorded in. Required with --commit at a monorepo workspace root, where the directory names the whole workspace rather than one releasable; rejected anywhere else, since the directory already names it, and rejected without --commit. |
...
| --- | --- |
| `supplied-release-commit` | All or none of `--version` (when supplied), `--commit` (when supplied). |
| `releasable-selects-a-supplied-commit` | `--releasable` requires `--commit`. |
