...
@release_group.command(
    ...
    consequential=True,
    help="Bring this repository's release archives into the three-fate model from its real history: record each version's release commit from the tag (or the historical spelling its archive names in shipped_as, or its version-bump commit), complete an archive whose required fields are missing or unanswered (a present but empty bump or description is unanswered), materialize an archive for a released version that never got one, and adopt a version tag no store records as the release it is evidence of. A reconstructed description comes from the first source that yields one -- an operator-reviewed --overrides file, the version's GitHub Release body, its CHANGELOG.md section, the commit subjects in its tag range -- and the archive names the source it came from. Every tag the repository cannot account for is listed first and refuses the whole apply; --dry-run prints the plan and writes nothing. With --version and --commit it does one thing instead: it records the commit a version shipped from on that version's archive marked unrecoverable (no tag and no version-bump commit named it), after verifying that the commit is an ancestor of HEAD and that every target in the release's scope declares the version at that commit (a workspace releasable: its .rlsbl-monorepo/releasables/<name>/version file). The mark is cleared and the commit and its tree hashes recorded exactly as the pass records a commit it found; an archive that is not marked unrecoverable is refused, so a recorded commit is never overwritten.",
    constraints=[
        # The supplied release commit names a version and the commit it shipped
        # from; either one alone says nothing the command can act on.
        strictcli.AllOrNone("supplied-release-commit", [
            strictcli.Member("version"),
            strictcli.Member("commit"),
        ]),
        # The whole-repository pass covers every releasable at once, so the
        # selector exists only for the one-archive form.
        strictcli.Requires("releasable-selects-a-supplied-commit", "releasable", "commit"),
    ],
)
...
@strictcli.flag(name="version", type=str, presence="optional", help="The version whose archive is marked unrecoverable and whose release commit --commit supplies (e.g. 0.4.2, with or without a leading v). Only together with --commit; the whole-repository pass does not run.")
@strictcli.flag(name="commit", type=str, presence="optional", help="The commit --version shipped from, as any spelling git resolves (a full or abbreviated hash). It must be an ancestor of HEAD, and every target in the release's scope must declare --version at it. Only together with --version.")
@strictcli.flag(name="releasable", type=str, presence="optional", help="Which releasable's archive --commit is recorded in. Required with --commit at a monorepo workspace root, where the directory names the whole workspace rather than one releasable; rejected anywhere else, since the directory already names it, and rejected without --commit.")
@effects.handler
def cmd_release_backfill(ctx, overrides, auto_commit, version, commit, releasable):
    """Backfill this repository's release archives from its real history."""
    _refuse_empty_flags(overrides=overrides, version=version, commit=commit,
    ...
    if commit is None:
        ...
        if overrides is not None:
            print(
                "Error: --overrides reviews the descriptions the whole-"
                "repository pass reconstructs; --version with --commit records "
                "one release commit and writes no description. Drop one.",
                file=sys.stderr,
            )
            sys.exit(1)
        # Resolved by MEMBERSHIP, exactly as `rlsbl release reconcile` resolves
        # the releasable it acts on: the archive belongs to one releasable.
        root = _require_sub_project_root()
        monorepo_root = find_workspace_root(str(root))
        if monorepo_root:
            from .workspace import (
            ...
            if _at_workspace_root(monorepo_root):
                project = _releasable_representative(
                    monorepo_root, releasable,
                    invocation="rlsbl release backfill", verb="backfill",
                )
            else:
                if releasable is not None:
                    _refuse_releasable_selector(
                        standalone=False, subject="backfilled",
                    )
            ...
            flags["releasable"] = resolve_releasable_for_project(
                project, load_releasables(monorepo_root, projects),
            ).name
        elif releasable is not None:
            _refuse_releasable_selector(standalone=True, subject="backfilled")
...
@mono.command(name="rename-releasable", help="Rename a releasable group. Rewrites the [[releasables]] name and every member's releasable field in workspace.toml (preserving comments), moves the state directory, drops the stale changelog validation cache, re-runs monorepo sync, and commits it all as one commit. That commit carries no Autogenerated trailer, so changelog coverage asks for an entry for it: the closing message prints the runnable rlsbl changelog add --type breaking line, changing into one of the releasable's members first. When tag_format contains {name}, a boundary alias tag for the current version is created at the old tag's commit and pushed; historical releases stay under the old prefix, and each one's archive records that tag in shipped_as so reconcile and release edit/deprecate/yank resolve it there. Idempotent: re-running heals a crash between the commit and the tag push, and records shipped_as on the past releases of a releasable renamed before the rename recorded it.", effect="mutating", consequential=True)
...
def cmd_mono_rename_releasable(ctx, old_name, new_name):
    ...
    if result.get("changelog_line"):
        print(
            "Add a breaking changelog entry for the rename commit, which "
            "changelog coverage asks for:\n  " + result["changelog_line"]
        )
