...
5. Commit all of the above as a single commit. It carries no
   ``Autogenerated`` trailer: changelog coverage asks for a breaking entry for
   it, and the closing message prints the ``rlsbl changelog add`` that adds it.
...
def _apply_local_rename(root, old, new):
    ...
    # Not autogenerated: the rename changes the spelling of every future tag
    # of the releasable, so changelog coverage asks for the breaking entry the
    # closing message prints (see :func:`changelog_add_line`).
    commit_files_if_changed(
