...
An ``unrecoverable`` archive is settled too, as far as this pass is concerned:
it never derives a commit for one again. The operator who knows the commit a
version shipped from supplies it with ``--version`` and ``--commit``, which
verifies it (see :func:`plan_supplied_commit`) and replaces the mark with the
recorded commit.
...
from .release_file import (
    ...
    replace_unrecoverable_with_release_commit,
...
# ---------------------------------------------------------------------------
# A caller-supplied release commit: --version <v> --commit <sha>
# ---------------------------------------------------------------------------


@dataclass
class SuppliedCommit:
    """The verified record of one caller-supplied release commit, ready to write."""
...
def _scope_for_supplied_commit(repo, releasable):
    """The release scope a supplied commit is recorded in.

    A standalone repository has one. In a workspace it is the releasable the
    command resolved (from ``--releasable`` at the workspace root, or from the
    member directory it ran in); the resolution is the command's, never this
    function's.
    """
    scopes = discover_scopes(repo)
    if releasable is None:
        if len(scopes) == 1 and scopes[0].label == "standalone":
            return scopes[0]
        raise BackfillError(
            "a workspace needs the releasable whose archive the commit is "
            "recorded in."
        )
    for scope in scopes:
        if getattr(scope.ref_ctx, "releasable_name", None) == releasable:
            return scope
    raise BackfillError(f"no releasable named {releasable!r} in this workspace.")


def _require_unrecoverable_archive(repo, scope, version, commit):
    """The archive path for *version*, which must be marked unrecoverable."""
    retry = f"--version {version} --commit {commit}"
    path = archived_versions(scope).get(version)
    if path is None:
        raise BackfillError(
            f"{version} has no archive in "
            f"{os.path.relpath(scope.releases_dir, repo)}, and --commit records "
            f"the release commit of an archive marked {UNRECOVERABLE_FIELD}. "
            f"If {version} was released, run `rlsbl release backfill` first: it "
            f"materializes the archive of every released version the changelog "
            f"records, and marks it {UNRECOVERABLE_FIELD} when no tag and no "
            f"version-bump commit names its commit. Then re-run with {retry}."
        )
    rel = os.path.relpath(path, repo)
    state = read_archive_state(path)
    if state.unrecoverable and not (state.recorded or state.never_released):
        return path
    if state.recorded:
        raise BackfillError(
            f"{rel} already records the release commit of {version} "
            f"(candidate_sha = {state.candidate_sha[:12] or '?'}), and a "
            f"recorded release commit is never overwritten."
        )
    if state.never_released:
        raise BackfillError(
            f"{rel} records {NEVER_RELEASED_FIELD} = true: no release was ever "
            f"published under {version}, so there is no release commit to "
            f"record."
        )
    raise BackfillError(
        f"{rel} is not marked {UNRECOVERABLE_FIELD}: it records no fate yet. "
        f"Run `rlsbl release backfill` first: it records the release commit "
        f"from a tag or a version-bump commit, or marks the archive "
        f"{UNRECOVERABLE_FIELD} when neither exists. If it is marked "
        f"{UNRECOVERABLE_FIELD}, re-run with {retry}."
    )


def _resolve_supplied_commit(repo, version, commit):
    """The full hash of *commit*, which must be in HEAD's history."""
    sha = rev_parse(repo, f"{commit}^{{commit}}")
    if not sha:
        raise BackfillError(
            f"--commit {commit!r} names no commit in this repository. Pass the "
            f"commit {version} shipped from: release commits are in the release "
            f"branch's history."
        )
    try:
        result = effects.run(
            ["git", "merge-base", "--is-ancestor", sha, "HEAD"],
            capture_output=True, text=True, timeout=GIT_TIMEOUT, cwd=repo,
        )
    except (subprocess.TimeoutExpired, OSError) as exc:
        raise BackfillError(f"cannot tell whether {sha[:12]} is in HEAD's history: {exc}") from exc
    if result.returncode != 0:
        raise BackfillError(
            f"{sha[:12]} is not an ancestor of HEAD: release commits are in the "
            f"release branch's history, so a commit outside it did not ship "
            f"{version}. Pass the commit {version} shipped from."
        )
    return sha


def _declared_versions_at(repo, scope, sha):
    """``[(declarer, declared-or-None, problem-or-None)]`` for the scope at *sha*.

    A workspace releasable declares its version in its own version file, read
    at *sha*. A standalone repository declares it in every target's manifest:
    the commit's files for the scope's released paths are materialized in a
    scratch directory and each target detected there answers through its own
    ``read_version``, the same reader the release flow uses.
    """
    releasable = getattr(scope.ref_ctx, "releasable_name", None)
    if releasable:
        rel = os.path.relpath(
            os.path.join(os.path.dirname(scope.releases_dir), "version"), repo,
        ).replace(os.sep, "/")
        try:
            text = effects.run(
                ["git", "show", f"{sha}:{rel}"],
        ...
        if text.returncode != 0:
            return [(rel, None, "the file does not exist there")]
        return [(rel, text.stdout.strip(), None)]

    from .errors import ConfigError
    from .targets import TARGETS, detect_targets

    try:
        with effects.observe_commit_files(
            repo, sha, scope.released_paths, timeout=GIT_TIMEOUT,
        ) as tree:
            # One released path per scope outside a releasable: "." for a
            # standalone repository, the member's own path for a member that
            # keeps its own release state.
            base = os.path.normpath(os.path.join(tree, scope.released_paths[0]))
            try:
                entries = detect_targets(base)
            except ConfigError as exc:
                raise BackfillError(
                    f"the release targets at {sha[:12]} cannot be read: {exc}"
                ) from exc
            declared = []
            for name, path in entries:
                ...
                if target is None:
                    continue
                try:
                    declared.append((name, target.read_version(path), None))
                except Exception as exc:  # any reader failure means "declares nothing readable"
                    declared.append((name, None, str(exc) or type(exc).__name__))
            return declared
    except subprocess.CalledProcessError as exc:
        detail = exc.stderr.decode(errors="replace").strip() if isinstance(exc.stderr, bytes) else exc.stderr
        raise BackfillError(f"cannot read the files of {sha[:12]}: {detail}") from exc


def _require_version_at(repo, scope, version, sha):
    """Refuse unless every declarer in the scope declares *version* at *sha*."""
    from .targets import TARGETS

    declared = _declared_versions_at(repo, scope, sha)
    if not declared:
        raise BackfillError(
            f"no release target is detected at {sha[:12]}, so nothing there "
            f"declares a version. Pass the commit {version} shipped from."
        )
    wrong = []
    for name, value, problem in declared:
        if problem is not None:
            wrong.append(f"{name} declares no readable version there ({problem})")
            continue
        target = TARGETS.get(name)
        if value == version or (target is not None and target.format_version(version) == value):
            continue
        wrong.append(f"{name} declares {value}")
    if wrong:
        raise BackfillError(
            f"refusing to record {sha[:12]} as the release commit of {version}: "
            f"at that commit {'; '.join(wrong)}, not {version}. Every target in "
            f"the release's scope declares the version it ships at the commit "
            f"it ships from. Pass the commit {version} shipped from."
        )


def plan_supplied_commit(repo, *, releasable, version, commit):
    """Verify a caller-supplied release commit. Raises :class:`BackfillError`.

    Nothing is written: :func:`apply_supplied_commit` writes what this returns.
    """
    scope = _scope_for_supplied_commit(repo, releasable)
    archive_path = _require_unrecoverable_archive(repo, scope, version, commit)
    sha = _resolve_supplied_commit(repo, version, commit)
    _require_version_at(repo, scope, version, sha)
    trees, notes = tree_hashes_at(repo, sha, scope.released_paths)
    if not trees:
        raise BackfillError(f"{sha[:12]} yields no tree for any released path.")
    return SuppliedCommit(
        repo=repo, scope=scope, version=version, archive_path=archive_path,
        candidate_sha=sha, tree_hashes=trees, notes=notes,
    )


def describe_supplied_commit(supplied):
    """The lines saying what :func:`apply_supplied_commit` writes."""
    rel = os.path.relpath(supplied.archive_path, supplied.repo)
    lines = [
        f"{supplied.scope.label} {supplied.version}: every target declares "
        f"{supplied.version} at {supplied.candidate_sha[:12]}",
        f"  {rel}: remove {UNRECOVERABLE_FIELD}, record "
        f"candidate_sha={supplied.candidate_sha[:12]} tree_hashes="
        f"{ {k: v[:12] for k, v in supplied.tree_hashes.items()} }",
    ]
    lines.extend(f"  note: {note}" for note in supplied.notes)
    return lines


def apply_supplied_commit(supplied):
    """Write the verified release commit. Returns the archive path written."""
    with writable_release_file(supplied.archive_path):
        replace_unrecoverable_with_release_commit(
            supplied.archive_path,
            candidate_sha=supplied.candidate_sha,
            tree_hashes=supplied.tree_hashes,
        )
    return supplied.archive_path


def supplied_commit_message(supplied, written):
    return (
        f"Record the release commit of {supplied.scope.label} "
        f"{supplied.version} in {written}"
    )
